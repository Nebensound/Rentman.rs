...
The generated sweep in `tests/live/generated_sweep.rs` calls every documented
`GET` collection endpoint without path parameters with `limit=1` and fails
when a live payload no longer matches the typed models — this catches drift
between Rentman's documentation and the real API. The sweep is regenerated by
